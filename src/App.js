...
function App() {
      ...
      if (dataFetched && data.response_code === 0) {
        ...
        setOptions(
          data.results.map((item, index) => ({
            ...
            answerOptions: item.incorrect_answers
              .map((answer) => ({
                answer: answer,
                question_id: index + 1,
                isCorrect: false,
                disabled: false,
                className: "trivia--answer_options_default",
              }))
              .concat({
  ...
  React.useEffect(() => {
    setOptions((prevState) => {
      return prevState.map((option) => {
        return {
          ...option,
          answerOptions: option.answerOptions.map((answer) =>
            answer.isCorrect === true
              ? {
                  ...answer,
                  className: "trivia--answer_options_true",
                  disabled: false,
                }
              : answer,
          ),
    ...
    setOptions((prevState) => {
      return prevState.map((option) => {
        return {
          ...option,
          answerOptions: option.answerOptions.map((answer) =>
            answer.disabled === false && answer.isCorrect === false
              ? {
                  ...answer,
                  className: "trivia--answer_options_false",
                  disabled: true,
                }
              : answer,
          ),
  ...
  function answerClick(e) {
    if (!score) {
      setOptions((prevState) => {
        return prevState.map((option) => {
          return option.id === parseInt(e.target.id)
            ? {
                ...option,
                answerOptions: option.answerOptions.map((answer) =>
                  answer.answer !== e.target.name
                    ...
                    : {
                        ...answer,
                        className: "trivia--answer_options_selected",
                      },
                ),
              }
            : option;
  ...
  function scoreCheck() {
    if (checkedAnswers.length === 10) {
      setScore(
        checkedAnswers
          .filter((isCorrect) => isCorrect.isCorrect === "true")
          .map((answer) => answer),
      );
    } else {
  ...
  function selectedCategory(selectedTopic) {
    !selectedTopic.id
      ? setCategory("")
      : setCategory("&category=" + selectedTopic.id);
  }
        ...
        <div>
          ...
          {!score ? (
            <div className="playagain--component">
              <button className="start--button" onClick={scoreCheck}>
                {" "}
                Check Answers{" "}
              </button>
            </div>
            ...
            <div className="playagain--component">
              <button className="playagain--button" onClick={restartGame}>
                {" "}
                Play Again{" "}
              </button>
              <div className="score">
                {" "}
                Your score: {score.length}/{correct.length}{" "}
              </div>
              <button className="playagain--button" onClick={changeTopic}>
                {" "}
                Change Topic{" "}
              </button>
            </div>
